...
func (c *Consumer) handleUpdateUserDetail(data []byte) {
	...
	if err := c.Store.UpdateUser(newUserData.Id, newUserData.Name, newUserData.Profile); err != nil {
		log.Println("Error when updating relation user data:", err)
	}
}
