...
func main() {
	...
	router.HandleFunc("/original/{filename}", library.CreateHandler(handleGetOriImage)).Methods(http.MethodGet)
...
func handleGetOriImage(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	status, err := getImage(w, r, "original")
	if err != nil {
		return status, err
	}
...
func handleGetThumbnailImage(w http.ResponseWriter, r *http.Request) (int, error) {
	log.Println("hit handleGetThumbnailImage")

	status, err := getImage(w, r, "thumbnail")
	if err != nil {
		return status, err
	}
...
func getImage(w http.ResponseWriter, r *http.Request, imageType string) (int, error) {
	vars := mux.Vars(r)
